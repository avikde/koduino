...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  ...
  FlashStatus = eraseHelper(OldPageAddress);
  /* If erase operation was failed, a Flash error code is returned */
  if (FlashStatus != FLASH_COMPLETE)
  {
    FLASH_Lock();
  ...
  // /* Flush the caches by setting the DCRST and ICRST bits in the FLASH_CR register. Note: The I/D cache should be flushed only when it is disabled (I/DCEN = 0).*/
