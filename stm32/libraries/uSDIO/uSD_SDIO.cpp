...
static SD_Error CmdResp1Error (uint8_t cmd, uint8_t *pstatus = NULL);
...
void SendSDIOCmd (uint32_t arg, uint32_t cmd, uint32_t resp = SDIO_Response_Short);
void SendDataInit (uint32_t dataLen, uint32_t blockSize, uint32_t transDir = SDIO_TransferDir_ToSDIO);
...
SD_Error SD_PowerON (void) {
	...
	SendSDIOCmd(0x0, SD_CMD_GO_IDLE_STATE, SDIO_Response_No);
	...
	/*!< CMD Response: R7 */
	SendSDIOCmd(SD_CHECK_PATTERN, SDIO_SEND_IF_COND);
	...
	if (errorstatus == SD_OK) {
		...
		/*!< CMD55 */
		SendSDIOCmd(0x00, SD_CMD_APP_CMD);
	...
	if (errorstatus == SD_OK) {
		...
		while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL )) {
			...
			SendSDIOCmd(SD_VOLTAGE_WINDOW_SD | SDType, SD_CMD_SD_APP_OP_COND);
...
SD_Error SD_InitializeCards (void) {
	...
	SendSDIOCmd(0x00, SD_CMD_ALL_SEND_CID, SDIO_Response_Long);
	...
	SendSDIOCmd(0x00, SD_CMD_SET_REL_ADDR);
	...
	SendSDIOCmd((uint32_t) (rca << 16), SD_CMD_SEND_CSD, SDIO_Response_Long);
	errorstatus = CmdResp2Error ();
	if (SD_OK != errorstatus) return (errorstatus);
 ...
 * @retval SD_Error: SD Card Error code.
 */
SD_Error SD_EnableWideBusOperation (void) {
	SD_Error errorstatus = SD_OK;

	errorstatus = SDEnWideBus();
...
SD_Error SD_SelectDeselect (uint64_t addr) {
	...
	SendSDIOCmd((uint32_t) addr, SD_CMD_SEL_DESEL_CARD);
...
SD_Error SD_ReadBlock (uint8_t *readbuff, uint64_t ReadAddr, uint16_t BlockSize) {
	...
	SendSDIOCmd((uint32_t) BlockSize, SD_CMD_SET_BLOCKLEN);
	errorstatus = CmdResp1Error (SD_CMD_SET_BLOCKLEN );
	if (SD_OK != errorstatus) return (errorstatus);

	SendDataInit(BlockSize, SDIO_DataBlockSize_512b);

	/*!< Send CMD17 READ_SINGLE_BLOCK */
	SendSDIOCmd((uint32_t) ReadAddr, SD_CMD_READ_SINGLE_BLOCK);
...
SD_Error SD_ReadMultiBlocks (uint8_t *readbuff, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks) {
	...
	SendDataInit(NumberOfBlocks*BlockSize, SDIO_DataBlockSize_512b);

	/*!< Send CMD18 READ_MULT_BLOCK with argument data address */
	SendSDIOCmd((uint32_t) ReadAddr, SD_CMD_READ_MULT_BLOCK);
...
SD_Error SD_WaitReadOperation (void) {
	SD_Error errorstatus = SD_OK;
	volatile uint32_t timeout = SD_DATATIMEOUT;
...
SD_Error SD_WriteBlock (uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize) {
	...
	SendSDIOCmd((uint32_t) WriteAddr, SD_CMD_WRITE_SINGLE_BLOCK);
	errorstatus = CmdResp1Error (SD_CMD_WRITE_SINGLE_BLOCK );
	if (errorstatus != SD_OK) return (errorstatus);

	SendDataInit(BlockSize, SDIO_DataBlockSize_512b, SDIO_TransferDir_ToCard);
...
SD_Error SD_WriteMultiBlocks (uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks) {
	...
	SendSDIOCmd((uint32_t) (RCA << 16), SD_CMD_APP_CMD);
	errorstatus = CmdResp1Error (SD_CMD_APP_CMD );
	if (errorstatus != SD_OK) return (errorstatus);

	/*!< To improve performance */
	SendSDIOCmd((uint32_t) NumberOfBlocks, SD_CMD_SET_BLOCK_COUNT);
	errorstatus = CmdResp1Error (SD_CMD_SET_BLOCK_COUNT );
	if (errorstatus != SD_OK) return (errorstatus);

	/*!< Send CMD25 WRITE_MULT_BLOCK with argument data address */
	SendSDIOCmd((uint32_t) WriteAddr, SD_CMD_WRITE_MULT_BLOCK);
	errorstatus = CmdResp1Error (SD_CMD_WRITE_MULT_BLOCK );
	if (SD_OK != errorstatus) return (errorstatus);

	SendDataInit(NumberOfBlocks*BlockSize, SDIO_DataBlockSize_512b, SDIO_TransferDir_ToCard);
...
SD_Error SD_StopTransfer (void) {
	...
	SendSDIOCmd(0x00, SD_CMD_STOP_TRANSMISSION);
...
SD_Error SD_Erase (uint64_t startaddr, uint64_t endaddr) {
	...
	SendSDIOCmd((uint32_t) startaddr, SD_CMD_SD_ERASE_GRP_START);
	errorstatus = CmdResp1Error (SD_CMD_SD_ERASE_GRP_START );
	if (errorstatus != SD_OK) return (errorstatus);

	/*!< Send CMD33 SD_ERASE_GRP_END with argument as addr  */
	SendSDIOCmd((uint32_t) endaddr, SD_CMD_SD_ERASE_GRP_END);
	errorstatus = CmdResp1Error (SD_CMD_SD_ERASE_GRP_END );
	if (errorstatus != SD_OK) return (errorstatus);

	/*!< Send CMD38 ERASE */
	SendSDIOCmd(0x00,SD_CMD_ERASE);
...
SD_Error SD_SendStatus (uint32_t *pcardstatus) {
	...
	if (pcardstatus == NULL) return (SD_INVALID_PARAMETER);

	SendSDIOCmd((uint32_t) RCA << 16, SD_CMD_SEND_STATUS);
...
SD_Error SD_SendSDStatus (uint32_t *psdstatus) {
	...
	SendSDIOCmd(64, SD_CMD_SET_BLOCKLEN);
	errorstatus = CmdResp1Error (SD_CMD_SET_BLOCKLEN );
	if (errorstatus != SD_OK) return (errorstatus);
	...
	SendDataInit(64, SDIO_DataBlockSize_64b);

	/*!< Send ACMD13 SD_APP_STAUS  with argument as card's RCA.*/
	SendSDIOCmd(0x00, SD_CMD_SD_APP_STAUS);
...
// Interrupt functions, need to be C since called from stm32f4xx_it.c
extern "C" {
	/**
 ...
 * @retval SD_Error: SD Card Error code.
 */
static SD_Error CmdResp1Error (uint8_t cmd, uint8_t *pstatus) {
	uint32_t status = SDIO->STA;
	...
	if (SDIO_GetCommandResponse () != cmd) return (SD_ILLEGAL_CMD);
	...
	if (pstatus != NULL) {
		...
		*pstatus = (uint8_t) ((response_r1 >> 9) & 0x0000000F);
	}
...
static SD_Error SDEnWideBus (void){
	...
	if ((scr[1] & SD_WIDE_BUS_SUPPORT )!= SD_ALLZERO) {
		...
		SendSDIOCmd(0x02, SD_CMD_APP_SD_SET_BUSWIDTH);
...
static SD_Error IsCardProgramming (uint8_t *pstatus) {
	SD_Error errorstatus = SD_OK;
	SendSDIOCmd((uint32_t) (RCA << 16), SD_CMD_SEND_STATUS);
	errorstatus = CmdResp1Error(SD_CMD_SEND_STATUS, pstatus);        
	return (errorstatus);
}
...
static SD_Error FindSCR (uint16_t rca, uint32_t *pscr) {
	...
	SendSDIOCmd(0x08, SD_CMD_SET_BLOCKLEN);
	errorstatus = CmdResp1Error (SD_CMD_SET_BLOCKLEN );
	if (errorstatus != SD_OK) return (errorstatus);
	...
	SendDataInit(8, SDIO_DataBlockSize_8b);

        /*!< Send ACMD51 SD_APP_SEND_SCR with argument as 0 */
	SendSDIOCmd(0x00, SD_CMD_SD_APP_SEND_SCR);
...
void SendSDIOCmd (uint32_t arg, uint32_t cmd, uint32_t resp) {
...
void SendDataInit (uint32_t dataLen, uint32_t blockSize, uint32_t transDir) {
	SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  SDIO_DataInitStructure.SDIO_DataLength = dataLen;
  SDIO_DataInitStructure.SDIO_DataBlockSize = blockSize;
  SDIO_DataInitStructure.SDIO_TransferDir = transDir;
